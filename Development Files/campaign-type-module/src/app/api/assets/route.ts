  ...
  try {
    ...
    return NextResponse.json({
      ...
      data: assets.map(a => ({
        ...
        unitCost:        a.unitCost ? Number(a.unitCost) : null,
        contributionPerMember: a.contributionPerMember ? Number(a.contributionPerMember) : null,
        positionStrategy: a.positionStrategy,
        allowOutsiders:  a.allowOutsiders,
        make:            a.make,
        model:           a.model,
        year:            a.year,
    ...
    const asset = await prisma.$transaction(async (tx) => {
      const a = await tx.asset.create({
        data: {
          ...
          notes:                data.notes,
          unitsTotal:           data.unitsTotal || 1,
          unitCost:             data.unitCost,
          contributionPerMember: data.contributionPerMember,
          positionStrategy:     data.positionStrategy || 'SENIORITY',
          allowOutsiders:       data.allowOutsiders || false,
        },
      })
