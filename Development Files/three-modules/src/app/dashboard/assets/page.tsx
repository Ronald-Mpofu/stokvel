...
import BackersPanel from './backers/page'
import IncomePanel from './income/page'
import InsurancePanel from './insurance/page'
import MaintenancePanel from './maintenance/page'
...
// ── Asset Detail ───────────────────────────────────────────────
function AssetDetail({ asset, members, onBack, onSuccess, onCosting, onQueue, onBackers, onIncome, onInsurance, onMaintenance }: any) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          ...
          {['ACQUIRED','ACTIVE'].includes(asset.status) && (
            <>
              <button onClick={() => onIncome && onIncome()} style={{ padding: '8px 14px', background: '#F0FDF4', color: '#166534', border: '1px solid #86EFAC', borderRadius: '8px', fontSize: '12px', cursor: 'pointer', fontWeight: '500' }}>💵 Income</button>
              <button onClick={() => onInsurance && onInsurance()} style={{ padding: '8px 14px', background: '#FEF9C3', color: '#854D0E', border: '1px solid #FCD34D', borderRadius: '8px', fontSize: '12px', cursor: 'pointer', fontWeight: '500' }}>🛡️ Insurance</button>
              <button onClick={() => onMaintenance && onMaintenance()} style={{ padding: '8px 14px', background: '#F3E8FF', color: '#6B21A8', border: '1px solid #C4B5FD', borderRadius: '8px', fontSize: '12px', cursor: 'pointer', fontWeight: '500' }}>🔧 Maintenance</button>
            </>
          )}
          <button onClick={() => onBackers && onBackers()} style={{ padding: '8px 14px', background: asset.allowOutsiders ? '#F0FDF4' : '#F1F5F9', color: asset.allowOutsiders ? '#166534' : '#64748B', border: `1px solid ${asset.allowOutsiders ? '#86EFAC' : '#E2E8F0'}`, borderRadius: '8px', fontSize: '12px', cursor: 'pointer', fontWeight: '500' }}>
            🌐 Backers {asset.allowOutsiders ? '' : '(disabled)'}
          </button>
          {(asset.campaignType === 'ROUND_ROBIN') && (
            <button onClick={() => onQueue && onQueue()} style={{ padding: '8px 14px', background: '#F3E8FF', color: '#6B21A8', border: '1px solid #C4B5FD', borderRadius: '8px', fontSize: '12px', cursor: 'pointer', fontWeight: '600' }}>🔄 Queue Manager</button>
          )}
        </div>
      </div>
...
export default function AssetsPage() {
  const [view, setView]         = useState<'list'|'create'|'detail'|'costing'|'queue'|'backers'|'income'|'insurance'|'maintenance'>('list')
  ...
  if (view === 'backers' && selected) return (
    <>
      {toast && <Toast msg={toast.msg} type={toast.type} onClose={() => setToast(null)} />}
      <BackersPanel
        asset={selected}
  ...
  if (view === 'income' && selected) return (
    <>
      {toast && <Toast msg={toast.msg} type={toast.type} onClose={() => setToast(null)} />}
      <IncomePanel asset={selected} onClose={() => setView('detail')} />
    </>
  )

  if (view === 'insurance' && selected) return (
    <>
      {toast && <Toast msg={toast.msg} type={toast.type} onClose={() => setToast(null)} />}
      <InsurancePanel asset={selected} onClose={() => setView('detail')} />
    </>
  )

  if (view === 'maintenance' && selected) return (
    <>
      {toast && <Toast msg={toast.msg} type={toast.type} onClose={() => setToast(null)} />}
      <MaintenancePanel asset={selected} onClose={() => setView('detail')} />
    </>
  )
  ...
  if (view === 'detail' && selected) return (
    <>
      ...
      <AssetDetail asset={selected} members={members} onBack={() => setView('list')}
        ...
        onBackers={() => setView('backers')}
        onIncome={() => setView('income')}
        onInsurance={() => setView('insurance')}
              ...
              <div key={a.id} style={{ background: 'white', borderRadius: '12px', border: `1px solid ${isRR ? '#C4B5FD' : '#E2E8F0'}`, overflow: 'hidden' }}>
                ...
                <div style={{ padding: '14px 16px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '6px' }}>
                    ...
                    <button onClick={() => { setSelected(a); setView('backers') }} title={a.allowOutsiders ? 'Outside Backers' : 'Outside Backers (disabled)'}
                      style={{ padding: '8px 10px', background: a.allowOutsiders ? '#F0FDF4' : '#F1F5F9', color: a.allowOutsiders ? '#166534' : '#94A3B8', border: 'none', borderRadius: '8px', fontSize: '12px', cursor: 'pointer' }}>🌐</button>
