// src/app/api/savings/route.ts — v2.2 (raw SQL fallback for new models)
import { NextRequest, NextResponse } from 'next/server'
import { z } from 'zod'
import prisma from '@/lib/prisma/client'

// Try generated client first, fall back to raw queries
async function safeModel(modelName: string) {
...
async function rawExec(sql: string, params: any[] = []) {
  try {
    return await prisma.$executeRawUnsafe(sql, ...params)
  } catch (e: any) {
    throw new Error(`Raw exec failed: ${e.message}`)
  }
}
...
function formatPool(p: any) {
  ...
  return {
    id:                   p.id,
    groupId:              p.groupId,
    groupName:            p.groupName || p.group?.name,
    currency:             p.currency || p.groupCurrency || 'USD',
    name:                 p.name,
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (poolId) {
      ...
      pool.members = (members as any[]).map(m => ({
      ...
      pool.loans = (loans as any[]).map(l => ({
        ...
        status: l.status, disbursedAt: l.disbursedAt, termMonths: l.termMonths,
        interestRatePct: (Number(l.interestRatePa)*100).toFixed(1),
      }))
      pool.payouts = (payouts as any[]).map(p => ({
...
export async function POST(req: NextRequest) {
  try {
    ...
    const group = await prisma.group.findUnique({
      where: { id: data.groupId }, select: { currency: true },
    })
    ...
    await rawExec(
      `INSERT INTO "SavingsPool" (
        ...
        $1,$2,$3,$4,$5,$6,$7,$8,$9,'SETUP',$10,$11,$12,$13,$14,0,0,0,NOW(),NOW()
      )`,
...
async function handleActivate(body: any): Promise<NextResponse> {
  const { poolId } = body
  const pools = await rawQuery(
    `SELECT sp.*, (SELECT json_agg(json_build_object('userId',"userId")) FROM "SavingsPoolMember" WHERE "poolId"=sp.id) as members
     FROM "SavingsPool" sp WHERE sp.id=$1`, [poolId]
  ) as any[]
      ...
      try {
        await rawExec(
          `INSERT INTO "SavingsContribution" (id,"poolId","userId","periodNumber","dueDate","amountDue","amountPaid",currency,status,"createdAt","updatedAt")
           VALUES ($1,$2,$3,$4,$5,$6,0,$7,'PENDING',NOW(),NOW()) ON CONFLICT ("poolId","userId","periodNumber") DO NOTHING`,
  ...
  await rawExec(`UPDATE "SavingsPool" SET status='ACTIVE',"updatedAt"=NOW() WHERE id=$1`, [poolId])
...
async function handleMature(body: any): Promise<NextResponse> {
  await rawExec(`UPDATE "SavingsPool" SET status='MATURED',"updatedAt"=NOW() WHERE id=$1`, [body.poolId])
...
async function handleDistribute(body: any): Promise<NextResponse> {
  const pools = await rawQuery(`SELECT * FROM "SavingsPool" WHERE id=$1`, [body.poolId]) as any[]
  if (!pools.length) return NextResponse.json({ success: false, error: 'Pool not found' }, { status: 404 })
  const pool = pools[0]
  ...
  await rawExec(`DELETE FROM "SavingsPoolPayout" WHERE "poolId"=$1 AND status='PENDING'`, [body.poolId])

  const members = await rawQuery(
    `SELECT * FROM "SavingsPoolMember" WHERE "poolId"=$1 AND "isActive"=true`, [body.poolId]
  ) as any[]
  ...
  for (const m of members) {
    ...
    await rawExec(
      `INSERT INTO "SavingsPoolPayout" (id,"poolId","userId","grossShare","loanDeduction","netPayout","sharePercent",currency,status,"createdAt")
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'PENDING',NOW()) ON CONFLICT ("poolId","userId") DO NOTHING`,
...
async function handlePayoutPaid(body: any): Promise<NextResponse> {
  ...
  const unpaid = await rawQuery(
    `SELECT COUNT(*) as cnt FROM "SavingsPoolPayout" WHERE "poolId"=$1 AND status='PENDING'`, [poolId]
  ) as any[]

  if (Number(unpaid[0].cnt) === 0) {
    await rawExec(`UPDATE "SavingsPool" SET status='CLOSED',"distributedAt"=NOW(),"updatedAt"=NOW() WHERE id=$1`, [poolId])
...
async function handleAddMember(body: any): Promise<NextResponse> {
  ...
  const pools = await rawQuery(`SELECT * FROM "SavingsPool" WHERE id=$1`, [poolId]) as any[]
  if (!pools.length) return NextResponse.json({ success: false, error: 'Pool not found' }, { status: 404 })
  const pool = pools[0]
  ...
  await rawExec(
    `INSERT INTO "SavingsPoolMember" (id,"poolId","userId","totalContributed","sharePercentage","loanBalance","isActive","createdAt","updatedAt")
     VALUES ($1,$2,$3,0,0,0,true,NOW(),NOW()) ON CONFLICT ("poolId","userId") DO UPDATE SET "isActive"=true,"exitedAt"=NULL,"updatedAt"=NOW()`,
    [memberId, poolId, userId]
  )
  ...
  const users = await prisma.user.findUnique({ where: { id: userId }, select: { fullName: true } })
  return NextResponse.json({ success: true, message: `${users?.fullName} added to pool` })
}
