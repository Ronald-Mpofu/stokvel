// src/app/api/savings/route.ts — v2.1
import { NextRequest, NextResponse } from 'next/server'
import { z } from 'zod'
import prisma from '@/lib/prisma/client'

// Helper — access new models safely regardless of client generation state
const db = {
  pool:         () => (prisma as any).savingsPool,
  member:       () => (prisma as any).savingsPoolMember,
  contribution: () => (prisma as any).savingsContribution,
  loan:         () => (prisma as any).savingsLoan,
  repayment:    () => (prisma as any).savingsLoanRepayment,
  payout:       () => (prisma as any).savingsPoolPayout,
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Create pool — NO $transaction to avoid proxy issues with new models
    ...
    // Step 2: Enrol initial members if provided
    if (data.memberIds.length > 0) {
      await db.member().createMany({
        data: data.memberIds.map((userId: string) => ({ poolId: pool.id, userId })),
        skipDuplicates: true,
      })
    }
...
async function handleActivate(body: any): Promise<NextResponse> {
  const { poolId } = body
  const pool = await db.pool().findUnique({ where: { id: poolId }, include: { members: true } })
  if (!pool)                   return NextResponse.json({ success: false, error: 'Pool not found' }, { status: 404 })
  ...
  for (let i = 0; i < contributions.length; i += batchSize) {
    await db.contribution().createMany({
      data: contributions.slice(i, i + batchSize),
      skipDuplicates: true,
    })
  }

  await db.pool().update({ where: { id: poolId }, data: { status: 'ACTIVE' } })
...
async function handleMature(body: any): Promise<NextResponse> {
  await db.pool().update({ where: { id: body.poolId }, data: { status: 'MATURED' } })
...
async function handleDistribute(body: any): Promise<NextResponse> {
  ...
  await db.payout().deleteMany({ where: { poolId: body.poolId, status: 'PENDING' } })

  const payouts = pool.members.filter((m: any) => m.isActive).map((m: any) => {
    const share  = Number(pool.totalContributed) > 0 ? Number(m.totalContributed) / Number(pool.totalContributed) : 1 / pool.members.length
    ...
    return {
      ...
      netPayout:     Math.max(0, gross - deduct),
      sharePercent:  share * 100,
      currency:      pool.currency,
    }
  })

  await db.payout().createMany({ data: payouts, skipDuplicates: true })
...
async function handlePayoutPaid(body: any): Promise<NextResponse> {
  ...
  const unpaid = await db.payout().count({ where: { poolId, status: 'PENDING' } })
  if (unpaid === 0) {
    await db.pool().update({ where: { id: poolId }, data: { status: 'CLOSED', distributedAt: new Date() } })
  }
...
async function handleAddMember(body: any): Promise<NextResponse> {
  ...
  if (pool.status === 'ACTIVE') {
    ...
    for (let p = 1; p <= periodCount; p++) {
      ...
      if (due >= now) {
        future.push({ poolId, userId, periodNumber: p, dueDate: due, amountDue: pool.contributionAmount, currency: pool.currency })
      }
    }
    if (future.length) await db.contribution().createMany({ data: future, skipDuplicates: true })
  }
