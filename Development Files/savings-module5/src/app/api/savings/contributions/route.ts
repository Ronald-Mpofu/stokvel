...
async function recalcPoolTotals(poolId: string) {
  const paid = await db.contribution().aggregate({
    where: { poolId, status: 'PAID' },
    _sum:  { amountPaid: true },
  })
  const interest = await db.loan().aggregate({
    where: { poolId },
  ...
  if (totalContrib > 0) {
    const members = await db.pool()Member.findMany({ where: { poolId } })
    const memberContribs = await db.contribution().groupBy({
