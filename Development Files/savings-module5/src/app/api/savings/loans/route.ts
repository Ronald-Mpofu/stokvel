...
export async function POST(req: NextRequest) {
  try {
    ...
    const loan = await prisma.$transaction(async (tx) => {
      ...
      await db.repayment().createMany({ data: schedule })
      return loan
    })
...
async function handleDisburse(body: any): Promise<NextResponse> {
  const loan = await db.loan().findUnique({ where: { id: body.loanId } })
...
async function handleRepay(body: any): Promise<NextResponse> {
  ...
  const [loan, repayment] = await Promise.all([
    db.loan().findUnique({ where: { id: loanId } }),
    db.repayment().findUnique({ where: { id: repaymentId } }),
  ])
