...
export async function POST(req: NextRequest) {
  try {
    ...
    await prisma.$transaction([
      prisma.savingsContribution.update({
        where: { id: data.contributionId },
        data: {
...
async function recalcPoolTotals(poolId: string) {
  const paid = await prisma.savingsContribution.aggregate({
    where: { poolId, status: 'PAID' },
    _sum:  { amountPaid: true },
  })
  const interest = await prisma.savingsLoan.aggregate({
    where: { poolId },
  ...
  if (totalContrib > 0) {
    const members = await prisma.savingsPoolMember.findMany({ where: { poolId } })
    const memberContribs = await prisma.savingsContribution.groupBy({
    ...
    for (const m of members) {
      ...
      await prisma.savingsPoolMember.update({
        where: { id: m.id },
