...
export async function GET(req: NextRequest) {
  try {
    ...
    const pools = await prisma.savingsPool.findMany({
      where,
      include: {
        group:   { select: { name: true, currency: true } },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const pool = await prisma.$transaction(async (tx) => {
      const pool = await tx.savingsPool.create({
        data: {
      ...
      if (data.memberIds?.length) {
        await tx.savingsPoolMember.createMany({
...
async function handleActivate(body: any): Promise<NextResponse> {
  ...
  await prisma.$transaction([
    prisma.savingsContribution.createMany({ data: contributions }),
    prisma.savingsPool.update({ where: { id: poolId }, data: { status: 'ACTIVE' } }),
  ])
...
async function handleMature(body: any): Promise<NextResponse> {
  await prisma.savingsPool.update({
...
async function handleDistribute(body: any): Promise<NextResponse> {
  ...
  await prisma.savingsPoolPayout.deleteMany({ where: { poolId: body.poolId, status: 'PENDING' } })
  ...
  await prisma.savingsPoolPayout.createMany({ data: payouts, skipDuplicates: true })
...
async function handlePayoutPaid(body: any): Promise<NextResponse> {
  ...
  await prisma.savingsPoolPayout.update({
    where: { poolId_userId: { poolId, userId } },
    data:  { status: 'PAID', paidAt: new Date(), paymentRef },
  })

  // Check if all paid → close pool
  const unpaid = await prisma.savingsPoolPayout.count({ where: { poolId, status: 'PENDING' } })
  if (unpaid === 0) {
    await prisma.savingsPool.update({ where: { id: poolId }, data: { status: 'CLOSED', distributedAt: new Date() } })
  }
...
async function handleAddMember(body: any): Promise<NextResponse> {
  ...
  if (pool.status === 'ACTIVE') {
    ...
    if (futureContribs.length) {
      await prisma.savingsContribution.createMany({ data: futureContribs, skipDuplicates: true })
    }
  }
