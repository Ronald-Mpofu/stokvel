...
export async function POST(req: NextRequest) {
  try {
    ...
    //
    // Same "paid AND not expired" computation as login — this is the
    // renewal clock's only reader. A member whose joiningFeeExpiresAt
    // has passed stops carrying the paid claim on their next refresh
    // (within 15 minutes, given the access-token expiry), with no cron.
    const rows: any[] = await prisma.$queryRawUnsafe(
      `SELECT "id","email","fullName","role","tier","kycStatus",
              "reputationScore","status","profilePhotoUrl",
