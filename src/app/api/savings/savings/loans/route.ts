...
export async function GET(req: NextRequest) {
  try {
    ...
    if (loanId) {
      const loan = await (prisma as any).savingsLoan.findUnique({
        where:   { id: loanId },
        include: {
    ...
    const loans = await (prisma as any).savingsLoan.findMany({
      where:   { poolId },
      include: {
        borrower:   { select: { fullName: true, email: true } },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const existing = await (prisma as any).savingsLoan.findFirst({
    ...
    const loan = await prisma.$transaction(async (tx) => {
      const loan = await (tx as any).savingsLoan.create({
        data: {
          poolId:            data.poolId,
      ...
      await (tx as any).savingsLoanRepayment.createMany({ data: schedule })
      return loan
    })
...
async function handleApprove(body: any): Promise<NextResponse> {
  await (prisma as any).savingsLoan.update({
...
async function handleRepay(body: any): Promise<NextResponse> {
  ...
  const [loan, repayment] = await Promise.all([
    (prisma as any).savingsLoan.findUnique({ where: { id: loanId } }),
    (prisma as any).savingsLoanRepayment.findUnique({ where: { id: repaymentId } }),
  ])
  ...
  await prisma.$transaction(async (tx) => {
    await (tx as any).savingsLoanRepayment.update({
    ...
    await (tx as any).savingsLoan.update({
      where: { id: loanId },
      data: {
    ...
    if (interestPortion > 0) {
      await (tx as any).savingsPool.update({
      ...
      await (tx as any).savingsPoolMember.updateMany({
