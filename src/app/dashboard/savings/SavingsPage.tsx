...
// ── Create Pool Modal ─────────────────────────────────────────
function CreatePoolModal({ groups, members, membersLoading, onClose, onSuccess, defaultGroupId }: any) {
  ...
  // Auto-select all group members when they load (if group already known)
  useEffect(() => {
    if (defaultGroupId && members.length > 0) {
      const ids = members.filter((m: any) => m.groupId === defaultGroupId).map((m: any) => m.userId)
      if (ids.length > 0) setForm(p => ({...p, memberIds: ids}))
    }
  }, [members.length, defaultGroupId])
        ...
        <form onSubmit={handleSubmit} style={{padding:'22px 24px'}}>
          ...
          {step===3&&<>
            ...
            {form.groupId&&groupMembers.length===0&&(
              membersLoading
                ? <div style={{background:'#F8FAFC',borderRadius:'8px',padding:'24px',textAlign:'center',color:'#94A3B8',fontSize:'13px'}}>
                    ⏳ Loading members...
                  </div>
                : <div style={{background:'#FEF9C3',borderRadius:'8px',padding:'16px',textAlign:'center',fontSize:'13px'}}>
                    <div style={{fontSize:'24px',marginBottom:'8px'}}>👥</div>
                    <div style={{color:'#854D0E',fontWeight:'600',marginBottom:'4px'}}>No members found for this group</div>
                    <div style={{color:'#92400E',fontSize:'12px'}}>Add members to the group first before creating a savings pool.</div>
                  </div>
            )}
            {form.groupId&&groupMembers.length>0&&<>
              {(() => {
          ...
          <div style={{display:'flex',gap:'10px',marginTop:'16px'}}>
            ...
            {step<3&&<button type="button" onClick={()=>setStep(s=>s+1)} disabled={step===1&&((!form.groupId&&!defaultGroupId)||!form.name||!form.contributionAmount)}
...
export default function SavingsPage({ groupId: propGroupId }: { groupId?: string }) {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const url = propGroupId ? `/api/savings?groupId=${propGroupId}` : '/api/savings'
      const [sRes, gRes] = await Promise.all([fetch(url), fetch('/api/groups')])
  ...
  }, [propGroupId])  // ← correct dependency

  // Fetch members lazily — only when create modal needs them
  const fetchMembers = useCallback(async () => {
    ...
    try {
      const groupsToFetch = propGroupId
        ...
        : groups
      if (!groupsToFetch.length) return
      const results = await Promise.all(
        groupsToFetch.map((g: any) =>
          fetch(`/api/members?groupId=${g.id}`)
            .then(r => r.json())
            .then(d => (d.success ? (d.data||[]).map((m: any) => ({
              ...m, groupId: g.id, userId: m.userId || m.id,
            })) : []))
            .catch(() => [])
        )
      )
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:'20px'}}>
      ...
      {showCreate&&<CreatePoolModal groups={groups} members={members} membersLoading={members.length===0} defaultGroupId={propGroupId} onClose={()=>setShowCreate(false)} onSuccess={(msg:string)=>{showToast(msg);fetchData()}}/>}
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        ...
        <div style={{display:'flex',gap:'8px'}}>
          ...
          <button onClick={()=>{ setShowCreate(true); fetchMembers() }} style={{padding:'10px 18px',background:TEAL,color:'white',border:'none',borderRadius:'8px',fontSize:'13px',fontWeight:'600',cursor:'pointer'}}>+ New Pool</button>
        </div>
      </div>
      ...
      {loading?(
        ...
        <div style={{background:'white',borderRadius:'12px',border:'1px solid #E2E8F0',padding:'60px',textAlign:'center'}}>
          ...
          <button onClick={()=>{ setShowCreate(true); fetchMembers() }} style={{padding:'10px 20px',background:TEAL,color:'white',border:'none',borderRadius:'8px',fontSize:'13px',fontWeight:'600',cursor:'pointer'}}>+ Create First Pool</button>
        </div>
      ):(
        <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
          {pools.map((p: any)=>{
