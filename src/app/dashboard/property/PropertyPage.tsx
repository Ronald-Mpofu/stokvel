...
function CreatePropertyModal({ groups, onClose, onSuccess, defaultGroupId }: any) {
  const [form, setForm] = useState({
    groupId:defaultGroupId||'', name:'', description:'', propertyAddress:'', propertyType:'residential',
...
export default function PropertyPage({ groupId: propGroupId }: { groupId?: string }) {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const pUrl = propGroupId ? `/api/property?groupId=${propGroupId}` : '/api/property'
      const [pRes, gRes] = await Promise.all([fetch(pUrl), fetch('/api/groups')])
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:'20px'}}>
      ...
      {showCreate&&<CreatePropertyModal groups={groups} defaultGroupId={propGroupId} onClose={()=>setShowCreate(false)} onSuccess={(msg:string)=>{showToast(msg);fetchData()}}/>}
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        ...
        <div style={{display:'flex',gap:'8px'}}>
          ...
          <button onClick={()=>{ setShowCreate(true); fetchMembers() }} style={{padding:'10px 18px',background:TEAL,color:'white',border:'none',borderRadius:'8px',fontSize:'13px',fontWeight:'600',cursor:'pointer'}}>+ Add Property</button>
        </div>
      </div>
