...
function ApplyModal({ groups, members, onClose, onSuccess, defaultGroupId }: any) {
  const [form, setForm] = useState({ groupId:defaultGroupId||'', borrowerId:'', type:'STANDARD', amount:'', interestRatePa:'0.24', termMonths:'12', purpose:'', isEmergency:false, guarantors:[] as {fullName:string;email:string;phone:string}[] })
...
function LoanDetail({ loanId, adminId, onClose, onAction }: any) {
  ...
  async function handleAction(action: string) {
    ...
    try {
      const res  = await fetch('/api/loans', { method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ loanId, action, reviewerId: adminId, ...reviewForm }) })
      const data = await res.json()
...
export default function LoansPage({ groupId: propGroupId }: { groupId?: string } = {}) {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const lUrl = propGroupId ? `/api/loans?groupId=${propGroupId}` : '/api/loans'
      const [lRes, gRes] = await Promise.all([fetch(lUrl), fetch('/api/groups')])
      ...
      const results = await Promise.all(
        groupsToFetch.map((g: any) =>
          fetch(`/api/members?groupId=${g.id}`)
            .then(r => r.json())
            .then(d => (d.success ? (d.data||[]).map((m: any) => ({...m, groupId: g.id})) : []))
            .catch(() => [])
        )
      )
  ...
  const filtered = loans.filter((l: any) => {
    ...
    const mg  = (propGroupId ? l.groupId === propGroupId : filterGroup === 'ALL' || l.groupId === filterGroup)
  ...
  return (
    <div style={{ display:'flex',flexDirection:'column',gap:'20px' }}>
      ...
      {showApply && <ApplyModal groups={groups} members={members} defaultGroupId={propGroupId} onClose={()=>setShowApply(false)} onSuccess={(msg:string)=>{showToast(msg);fetchData()}} />}
      ...
      <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center' }}>
        ...
        <div style={{ display:'flex',gap:'8px' }}>
          ...
          <button onClick={()=>{ setShowApply(true); fetchMembers() }} style={{ padding:'10px 18px',background:TEAL,color:'white',border:'none',borderRadius:'8px',fontSize:'13px',fontWeight:'600',cursor:'pointer' }}>+ New Application</button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ background:'white',borderRadius:'12px',border:'1px solid #E2E8F0',padding:'60px',textAlign:'center' }}>
          ...
          {loans.length===0&&<button onClick={()=>{ setShowApply(true); fetchMembers() }} style={{ padding:'10px 20px',background:TEAL,color:'white',border:'none',borderRadius:'8px',fontSize:'13px',fontWeight:'600',cursor:'pointer',marginTop:'12px' }}>+ New Application</button>}
        </div>
      ) : (
